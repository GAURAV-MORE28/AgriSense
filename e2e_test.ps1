...
Write-Output ('TOP_SCHEME_ID:' + $schemes.recommendations[0].scheme.scheme_id)

Write-Output '=== SUBMIT APPLICATION ==='
$appPayload = @{ profile_id = $profileId; scheme_id = $schemes.recommendations[0].scheme.scheme_id; scheme_name = $schemes.recommendations[0].scheme.name; documents = @(); form_data = @{ name = $profRes.profile.name; state = $profRes.profile.state }; client_timestamp = (Get-Date).ToString('o') }
